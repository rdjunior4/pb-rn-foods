      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3 lg:gap-4">
        {displayItems.map((b, i) => {
          ...
          return (
            <div
              key={b.id || i}
              className="group relative flex flex-col items-center text-center rounded-lg bg-gradient-to-b from-primary to-primary-hover p-3 sm:p-5 lg:p-7 hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/30 transition-all duration-300 overflow-hidden"
            >
              ...
              <div className="inline-flex h-10 w-10 sm:h-14 sm:w-14 items-center justify-center rounded-lg bg-gradient-to-b from-white/20 to-white/5 text-white backdrop-blur-sm mb-2.5 sm:mb-4 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-white/20 transition-all duration-300 ring-1 ring-white/15">
                <IconComp className="h-5 w-5 sm:h-6 sm:w-6" />
              </div>
              <div className="font-semibold text-xs sm:text-sm text-white relative">{b.title}</div>
              <div className="text-[10px] sm:text-xs text-white/60 mt-1 sm:mt-1.5 leading-relaxed max-w-[160px] sm:max-w-[200px] relative">{b.desc}</div>
